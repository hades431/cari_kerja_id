        ...
        <aside class="bg-gradient-to-b from-teal-700 to-teal-900 w-64 flex flex-col shadow-xl fixed inset-y-0 left-0">
            ...
            <nav class="flex-1 flex flex-col gap-1 px-2 mt-4">
                <a href="../dashboard/dashboard.php"
                    class="flex items-center gap-3 px-6 py-3 rounded-lg font-medium transition <?= $menuAktif['dashboard'] ? 'bg-teal-900 text-white' : 'text-teal-100 hover:bg-teal-900 hover:text-white' ?>">
                ...
                <a href="../pelamar/pelamar.php"
                    class="flex items-center gap-3 px-6 py-3 rounded-lg font-medium transition <?= $menuAktif['pelamar'] ? 'bg-teal-900 text-white' : 'text-teal-100 hover:bg-teal-900 hover:text-white' ?>">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                ...
                <a href="../perusahaan/perusahaan.php"
                    class="flex items-center gap-3 px-6 py-3 rounded-lg font-medium transition <?= $menuAktif['perusahaan'] ? 'bg-teal-900 text-white' : 'text-teal-100 hover:bg-teal-900 hover:text-white' ?>">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                ...
                <a href="../transaksi/riwayat_transaksi.php"
                    class="flex items-center gap-3 px-6 py-3 rounded-lg font-medium transition <?= $menuAktif['transaksi'] ? 'bg-teal-900 text-white' : 'text-teal-100 hover:bg-teal-900 hover:text-white' ?>">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round"
                ...
                <a href="../lowongan_kerja/lowongan_kerja.php"
                    class="flex items-center gap-3 px-6 py-3 rounded-lg font-medium transition <?= $menuAktif['lowongan'] ? 'bg-teal-900 text-white' : 'text-teal-100 hover:bg-teal-900 hover:text-white' ?>">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round"
                ...
                <a href="../tips_kerja/tips_kerja_artikel.php"
                    class="flex items-center gap-3 px-6 py-3 rounded-lg font-medium transition <?= $menuAktif['artikel'] ? 'bg-teal-900 text-white' : 'text-teal-100 hover:bg-teal-900 hover:text-white' ?>">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                ...
                <div class="flex items-center justify-between mb-6">
                    ...
                    <form method="GET" class="flex items-center gap-2">
                        <input type="text" name="search" value="<?= htmlspecialchars($keyword) ?>"
                            ...
                            class="px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-teal-500">
                        <button type="submit"
                            class="bg-teal-600 hover:bg-teal-700 text-white px-4 py-2 rounded-lg shadow">Cari</button>
                    </form>
                </div>
                    ...
                    <table class="w-full text-left border-collapse">
                        ...
                        <tbody class="divide-y divide-gray-200">
                            ...
                            <tr class="hover:bg-gray-50 transition">
                                ...
                                <td class="px-4 py-3 font-medium text-gray-700"><?= htmlspecialchars($row['nama']); ?>
                                </td>
                                <td class="px-4 py-3">
                                    ...
                                    <form action="update_role.php" method="POST" class="flex items-center gap-2">
                                        ...
                                        <select name="role" class="px-2 py-1 border rounded-lg text-sm text-gray-700">
                                            <option value="pelamar" <?= $row['role'] == 'Pelamar' ? 'selected' : '' ?>>
                                                Pelamar</option>
                                            <option value="perusahaan"
                                                <?= $row['role'] == 'Perusahaan' ? 'selected' : '' ?>>Perusahaan
                                            </option>
                                        </select>
                                ...
                                <td class="px-4 py-3">
                                    <span
                                        class="px-2 py-1 rounded-full text-xs font-semibold <?= $row['status_akun'] == 'aktif' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700' ?>"><?= $row['status_akun']; ?></span>
                                </td>
                                <td class="px-4 py-3 text-center">
                                    ...
                                    <a href="update_status.php?id=<?= $row['id_user']; ?>&status=nonaktif"
                                        class="bg-red-600 hover:bg-red-700 text-white px-4 py-1.5 rounded-lg text-sm shadow">Nonaktifkan</a>
                                    <?php else: ?>
                                    <a href="update_status.php?id=<?= $row['id_user']; ?>&status=aktif"
                                        class="bg-green-600 hover:bg-green-700 text-white px-4 py-1.5 rounded-lg text-sm shadow">Aktifkan</a>
                                    <?php endif; ?>
                                    <?php endif; ?>
